...
Trace Triangle::hit(const Ray& ray) const {
    ...
    if(u >= 0 && u <= 1 && v >= 0 && (v <= 1) & (u + v <= 1)) {
        /*
        we can express the position of P in t, u, v space. 
        t indicates the distance from P to the ray origin and is parallel to the y-axis.
        //表示t是P到射线原点的距离
        if P lies in the unit triangle it means that 0 <= u <= 1, 0 <= v <= 1 and u+v <=1.
        */
        if(t >= ray.dist_bounds.x && t <= ray.dist_bounds.y) {
            
            // calculate each parameter for Trace::ret
            //包括 origin, hit,distance, position, normal
            ret.hit = true; //表示命中
            //如果射线与三角形相交，交点处的法线方向应该由交点处的重心坐标求得：
           

            ret.normal = u * v_0.normal + v * v_1.normal + (1 - u - v) * v_2.normal;
            // ret.normal = cross(e1, e2);// what was the surface normal at the intersection?

            ret.origin = ray.point;
            Vec3 refl_dir = ray.dir - 2.0f * dot(ret.normal, ray.dir) * ret.normal; //表示反射的方向
