...
std::optional<Halfedge_Mesh::EdgeRef> Halfedge_Mesh::flip_edge(Halfedge_Mesh::EdgeRef e) {
    ...
    //翻转过程不需要创建或删除元素？，其实重新连接完，就相当于消失了，即删除
    //以下为测试：
    /* info("---start flipping edge %u---", e->id());
    info("(v3, v4) (%u, %u)", v3->id(), v4->id());
    info("face 1 3 4: %u->%u->%u", f1->halfedge()->vertex()->id(),
         f1->halfedge()->next()->vertex()->id(), f1->halfedge()->next()->next()->vertex()->id());
    info("face 2 4 3: %u->%u->%u", f2->halfedge()->vertex()->id(),
         f2->halfedge()->next()->vertex()->id(), f2->halfedge()->next()->next()->vertex()->id());
    info("---end---");*/
    // 其余部分请大家自己完成
    return e;
...
std::optional<Halfedge_Mesh::VertexRef> Halfedge_Mesh::split_edge(Halfedge_Mesh::EdgeRef e) {
    /*任务：
    将指定的边 e 分裂成两条边，在新增顶点和 e
    相对的顶点之间新增一条边，并返回被分裂的
    边。分裂过程中只能新增元素，不能删除元素；
    新增顶点位于 e 两端点中间。
    */
    //分裂e后，需要创建的是两个halfedge，两个edge，两个新面
    //疑问，一次处理一个三角形还是两个???
    ...
    HalfedgeRef h_twin = h->twin();
    HalfedgeRef h_1_4 = h_twin->next();
    HalfedgeRef h_3_2 = h->next();
    HalfedgeRef h_2_1 = h->next()->next();
    HalfedgeRef h_4_3 = h_twin->next()->next();
    //需要的点
    VertexRef v1 = h->vertex();
    VertexRef v2 = h->next()->next()->vertex();
    VertexRef v3 = h_twin->vertex();
    VertexRef v4 = h_twin->next()->next()->vertex();
    //需要的面
    FaceRef f1 = h->face();
    FaceRef f2 = h_twin->face();
    ...
    //以下开始创建元素
    //创建点
    VertexRef vNew = new_vertex(); //用输入变量吗？
    /*float vNew_x = (v1->pos.x - v3->pos.x) / 2 + v3->pos.x;
    float vNew_y = (v1->pos.y - v3->pos.y) / 2 + v3->pos.y;
    float vNew_z = (v1->pos.z - v3->pos.z) / 2 + v3->pos.z;
    vNew->pos = Vec3(vNew_x, vNew_y, vNew_z);*/
    vNew->pos = e->center(); //效果是一样的
    // vNew->halfedge()=后面要补
    vNew->is_new = true;

    //创建两个双边，由分裂而来的边
    HalfedgeRef h_twin_newH = new_halfedge();
    HalfedgeRef h_newTwin = new_halfedge();
    //创建两个面，用作分裂后的面
    FaceRef f4 = new_face();
    FaceRef f3 = new_face();
    //创建一个边，原来的边用于h，新创建的边用于h_twin
    EdgeRef enew = new_edge();
    //创建两个边，用于连接2，vnew和4，vnew，且相应的四个半边创建
    EdgeRef e2 = new_edge();
    EdgeRef e4 = new_edge();

    e2->halfedge() = new_halfedge();
    HalfedgeRef h_v_2 = e2->halfedge();
    h_v_2->twin() = new_halfedge();
    HalfedgeRef h_v_2_twin = h_v_2->twin();

    e4->halfedge() = new_halfedge();
    HalfedgeRef h_v_4 = e4->halfedge();
    h_v_4->twin() = new_halfedge();
    HalfedgeRef h_v_4_twin = h_v_4->twin();

    //下面开始处理新创建的参数

    //两个双边的设置
    h_twin_newH->set_neighbors(h_v_2, h_twin, v1, enew, f3);
    h_newTwin->set_neighbors(h_v_4, h, v3, e, f4);
    // enew的设置
    enew->halfedge() = h_twin;
    //产生的两个边的四个半边对应的配置：
    h_v_2->set_neighbors(h_2_1, h_v_2_twin, vNew, e2, f3);
    h_v_2_twin->set_neighbors(h, h_v_2, v2, e2, f1);
    h_v_4->set_neighbors(h_4_3, h_v_4_twin, vNew, e4, f4);
    h_v_4_twin->set_neighbors(h_twin, h_v_4, v4, e4, f2);
    //两个创建面对应半边的设置
    f3->halfedge() = h_twin_newH;
    f4->halfedge() = h_newTwin;
    //补上vnew的halfedge
    vNew->halfedge() = h_v_2;
    ...
    h_twin->next() = h_1_4;
    h_twin->twin() = h_twin_newH;
    h_twin->edge() = enew;
    h_twin->vertex() = vNew;

    //修改原有边对应next变化
    h_3_2->next() = h_v_2_twin;
    h_2_1->next() = h_twin_newH;
    h_1_4->next() = h_v_4_twin;
    h_4_3->next() = h_newTwin;

    //修改原有边对应面的变化
    h_2_1->face() = f3;
    h_4_3->face() = f4;

    //修改原有边=面对应边的变化（虽然是随机，但是重新赋值比较好）
    f1->halfedge() = h;
    f2->halfedge() = h_twin;
    //修改原有的点对应的边的变化
    v1->halfedge() = h_1_4;
    ...
    //进行边的属性更改
    // edge is_new属性设置，需要根据loop主函数进行设置
    enew->is_new = false;
    // e->is_new = false;
    e2->is_new = true;
    e4->is_new = true;
    //最后将vNew更新位置
    //info("e:%u new_pos%lf %lf %lf", e->id(), e->new_pos.x, e->new_pos.y, e->new_pos.z);
    vNew->pos = e->new_pos;

    //以下输出所有数据检查
    /* info("---start spilit edge %u---", e->id());
    info("(vnew)(%u)", vNew->id());
    info("vnewpos:%u", vNew->pos);
    info("(v1, v2) (%u, %u)", v1->id(), v2->id());
    info("(v3, v4) (%u, %u)", v3->id(), v4->id());
    info("f1:", f1->id());
    info("face 1: %u->%u->%u", f1->halfedge()->vertex()->id(),
         f1->halfedge()->next()->vertex()->id(), f1->halfedge()->next()->next()->vertex()->id());
    info("f2:", f2->id());
    info("face 2: %u->%u->%u", f2->halfedge()->vertex()->id(),
         f2->halfedge()->next()->vertex()->id(), f2->halfedge()->next()->next()->vertex()->id());
    info("f3:", f3->id());
    info("face 3: %u->%u->%u", f3->halfedge()->vertex()->id(),
         f3->halfedge()->next()->vertex()->id(), f3->halfedge()->next()->next()->vertex()->id());
    info("f4:", f4->id());
    info("face 4: %u->%u->%u", f4->halfedge()->vertex()->id(),
         f4->halfedge()->next()->vertex()->id(), f4->halfedge()->next()->next()->vertex()->id());
    *//*
    //半边检查
    info("h_1_4:%u", h_1_4->face()->id());
    info("h_2_1:%u", h_2_1->face()->id());
    info("h_4_3:%u", h_4_3->face()->id());
    info("h_3_2:%u", h_3_2->face()->id());
    info("h:%u", h->face()->id());
    info("h_twin:%u", h_twin->face()->id());
    info("h_twin_newH:%u", h_twin_newH->face()->id());
    info("h_newTwin:%u", h_newTwin->face()->id());
    info("%u", h_v_2->face()->id());
    info("%u", h_v_2_twin->face()->id());
    info("%u", h_v_4->face()->id());
    info("%u", h_v_4_twin->face()->id());
    //边的检查
    //发现边的id打不出来
    //enew出问题
    info("enew:%u", enew->halfedge()->id());
    info("enew:%u", enew->id());
    info("e2:%u", e2->halfedge()->id());
    info("e2:%u", e2->id());
    info("e4:%u", e4->halfedge()->id());
    info("e4:%u", e4->id());*/
    return vNew;
...
void Halfedge_Mesh::loop_subdivide() {
    ...
    for(EdgeRef edge = edges_begin(); edge != edges_end(); edge++) {
        edge->is_new = false; //全局设为false
        // edge->new_pos=edge->center();不对，这个是split里需要的中点
        // edge存的new_pos是3/8的那个
        edge->new_pos =
            (edge->halfedge()->vertex()->pos + edge->halfedge()->twin()->vertex()->pos) * 0.375 +
            (edge->halfedge()->next()->next()->vertex()->pos +
             edge->halfedge()->twin()->next()->next()->vertex()->pos) *
                0.125;
         //info("edgecenter:pos:%lf %lf %lf", edge->center().x, edge->center().y,edge->center().z);
         //info("edge:%u new_pos%lf %lf %lf", edge->id(), edge->new_pos.x, edge->new_pos.y,
         //edge->new_pos.z);
    }
    //遍历点
    for(VertexRef vertex = vertices_begin(); vertex != vertices_end(); vertex++) {
        ...
        Vec3 sumNeighbor = Vec3(0);int n = 0;
        do {
            VertexRef neighbor = h->twin()->vertex();
            // 对邻接顶点做一些操作
            sumNeighbor += neighbor->pos;
            h = h->twin()->next();
            n++;
        } while(h != v->halfedge());
        info("sumNeighbor pos:%lf %lf %lf", sumNeighbor.x, sumNeighbor.y, sumNeighbor.z);
        info("n=%lf", n);
        //n++; //???
        if(n == 3)
            float(7) / float(16) * vertex->pos + float(3) / float(16) * sumNeighbor;
        else
            vertex->new_pos = float(5) / float(8) * vertex->pos + float(3) / float(8)/float(n) * sumNeighbor;
        info("v:%u original_pos%lf %lf %lf", v->id(), v->pos.x, v->pos.y, v->pos.z);
        info("v:%u new_pos%lf %lf %lf", v->id(), v->new_pos.x, v->new_pos.y, v->new_pos.z);
    }

    for(EdgeRef edge = edges_begin(); edge != edges_end(); edge++)
        if(edge->halfedge()->vertex()->is_new == false &&
           edge->halfedge()->twin()->vertex()->is_new == false) {
            split_edge(edge);
            info("edge:%u", edge->id());
        }

    for(EdgeRef edge = edges_begin(); edge != edges_end(); edge++)
        if(edge->is_new == true && (edge->halfedge()->vertex()->is_new == false ||
                                    edge->halfedge()->twin()->vertex()->is_new == false)) {
            flip_edge(edge);
            info("flip_edge:%u", edge->id());
        }

     for(VertexRef vertex = vertices_begin(); vertex != vertices_end();
             vertex++) { //凸出来的都是老点，查询计算的坐标状态
        if(vertex->is_new == false) {
            info("vertex:%u", vertex->id());
